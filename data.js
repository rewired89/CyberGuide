...

  {
    ...
    concepts: [
...

      {
        ...
        tags: ["sec", "arch"],
        chain: ["Signer hashes the message", "Hash encrypted with private key", "Signature attached to message", "Receiver decrypts with public key", "Hashes match = authentic and unmodified"],
        blurb: "A digital signature proves two things: who sent a message, and that it hasn't been changed since. It's the cryptographic foundation of trust on the internet.",
        detail: `<strong>How signing works:</strong>\n1. Compute a hash of the data (SHA-256 output = 32 bytes)\n2. Encrypt that hash with your private key → this is the signature\n3. Send data + signature\n4. Receiver computes hash of data independently\n5. Receiver decrypts signature with your public key → gets original hash\n6. If both hashes match: data is authentic and unmodified\n\nImportant: you're not encrypting the data, just the hash. Anyone can read the data. The signature just proves it came from you and wasn't altered.\n\n<strong>Properties a digital signature provides:</strong>\n• Authenticity: only the holder of the private key could have signed it\n• Integrity: any change to the data invalidates the signature\n• Non-repudiation: signer can't deny signing — only they had the private key\n\n<strong>Signature algorithms:</strong>\n• RSA-PSS: RSA with Probabilistic Signature Scheme padding. Current RSA signing standard (PKCS#1 v1.5 is older, avoid for new systems).\n• DSA (Digital Signature Algorithm): based on discrete log. Requires good randomness — bad RNG = private key exposed. Being phased out.\n• ECDSA: DSA on elliptic curves. Smaller signatures. Same RNG risk as DSA.\n• EdDSA / Ed25519: deterministic — doesn't need a random nonce per signature. Immune to the RNG problem that broke DSA/ECDSA in PlayStation 3 and Android Bitcoin wallets. Fast, simple, preferred for new systems.\n\n<strong>Code signing:</strong>\nSoftware publishers sign binaries with their private key. Your OS verifies the signature before running the code. If it doesn't match, the binary was tampered with after signing — malware alert.`,
        memory: `Digital signature = wax seal on a letter. Only you have the signet ring (private key). Anyone can verify the seal's authenticity (public key). If the seal is broken or wrong, the letter was tampered with.\n\nEd25519 = the seal that can't be forged even if someone knows your random number generator is weak.`,
        examTip: `Signature = hash encrypted with private key. Verify = decrypt with public key, compare hashes. Non-repudiation = signer can't deny it. Ed25519 = preferred modern algorithm (deterministic, no RNG risk). ECDSA = watch for RNG dependency. DSA = being phased out. RSA-PSS = preferred over PKCS#1 v1.5.`,
        facts: ["Sign=encrypt hash with priv key", "Verify=pub key decrypts hash", "Non-repudiation guaranteed", "Ed25519=preferred", "ECDSA=RNG dependent", "Code signing=binary integrity"]
      },

      {
        id: "tls-handshake",
        title: "TLS 1.3 Handshake — Every Step",
        tags: ["sec", "arch"],
        chain: ["Client Hello (supported ciphers)", "Server Hello + certificate", "Key exchange (ECDHE)", "Handshake verified", "Encrypted application data flows"],
        blurb: "TLS 1.3 is what makes HTTPS secure. Understanding every step of the handshake tells you exactly where the trust, key material, and encryption come from.",
        detail: `<strong>TLS 1.3 handshake (1-RTT — one round trip):</strong>\n\n<strong>Step 1 — Client Hello:</strong>\nClient sends: TLS version, list of supported cipher suites, a random nonce, and its ECDHE key share (the public part of an ephemeral key pair it just generated).\n\n<strong>Step 2 — Server Hello:</strong>\nServer picks the cipher suite (e.g., TLS_AES_256_GCM_SHA384). Sends its own ECDHE key share. Both sides now compute the shared secret independently (ECDHE).\n\n<strong>Step 3 — Key Derivation:</strong>\nBoth sides run the shared secret + nonces through HKDF (HMAC-based Key Derivation Function) to derive:\n• Handshake keys (to encrypt the rest of the handshake)\n• Application keys (to encrypt actual data)\n\n<strong>Step 4 — Server Certificate + Verify:</strong>\nServer sends its certificate (encrypted with handshake keys). Client verifies: is the cert signed by a trusted CA? Does the domain match? Is it expired? Is it revoked?\nServer sends a signature over the entire handshake transcript — proves the server holds the private key matching the cert's public key.\n\n<strong>Step 5 — Finished:</strong>\nBoth sides send a MAC over the handshake transcript to confirm nothing was tampered with during negotiation.\n\n<strong>Application data:</strong>\nNow flows encrypted with the derived application keys (AES-256-GCM or ChaCha20-Poly1305).\n\n<strong>TLS 1.3 vs 1.2 improvements:</strong>\n• 1-RTT handshake (vs 2-RTT in TLS 1.2) = faster\n• 0-RTT resumption for repeat connections (but replay attack risk)\n• Removed all weak ciphers (RC4, DES, 3DES, CBC mode, RSA key exchange)\n• ECDHE required — PFS mandatory\n• Encrypts more of the handshake (certificate is now encrypted)\n• Removed renegotiation (attack surface in TLS 1.2)\n\n<strong>Cipher suite anatomy:</strong>\nTLS_AES_256_GCM_SHA384\n→ Key exchange: ECDHE (implicit in TLS 1.3)\n→ Authentication: certificate-based (implicit)\n→ Symmetric cipher: AES-256-GCM\n→ Hash function: SHA-384 (for HKDF and MAC)`,
        memory: `TLS 1.3 = two people agreeing on a secret language while a crowd watches, then switching to that language in under a second. Nobody watching can understand what they're saying or replay the conversation later.\n\n1.2 vs 1.3: 1.2 = negotiating with all options on the table (including bad ones). 1.3 = only good options, faster, more of it encrypted.`,
        examTip: `TLS 1.3 = 1-RTT, ECDHE mandatory, PFS built-in, AEAD only. TLS 1.0/1.1 = deprecated (RFC 8996). SSL = dead. "What provides PFS in TLS?" → ECDHE. Certificate proves server identity. ECDHE provides the session keys. They're independent — this separation is intentional.`,
        facts: ["TLS 1.3=1-RTT", "ECDHE=mandatory", "PFS=built-in", "AEAD ciphers only", "Certificate=encrypted in 1.3", "0-RTT=replay risk", "TLS 1.0/1.1=deprecated"]
      },

      {
        id: "cert-transparency-ocsp",
        title: "Certificate Transparency & OCSP Stapling",
        tags: ["sec", "arch"],
        chain: ["CA issues certificate", "Logged in public CT log", "Browser checks log proof", "OCSP confirms not revoked", "Connection trusted"],
        blurb: "Two mechanisms that close gaps in the certificate system: CT logs catch misissuance publicly, OCSP stapling checks revocation without leaking your browsing to the CA.",
        detail: `<strong>The problem CT solves — misissuance:</strong>\nA CA can issue a certificate for any domain, to anyone. Historically, this happened (DigiNotar issued certs for Google to the Iranian government in 2011). There was no way to detect rogue certs until damage was done.\n\n<strong>Certificate Transparency (CT):</strong>\nRequires CAs to log every certificate they issue to public, append-only CT logs before browsers will trust it. Browsers check that a valid Signed Certificate Timestamp (SCT) exists from a recognized log.\nResult: every certificate is publicly visible. Anyone can monitor for certs issued for their domain. If a rogue cert appears, it can be detected and revoked quickly.\nTools: crt.sh lets you see every cert ever issued for a domain.\n\n<strong>Certificate Pinning:</strong>\nAn app hardcodes which specific certificate or public key it will accept. Even a valid CA-signed cert is rejected if it doesn't match the pin. Protects against CA compromise.\nRisk: if you rotate your cert and forget to update the pin, your app breaks. Google removed Chrome pinning for this reason (HPKP deprecated). Still used in mobile apps.\n\n<strong>OCSP (Online Certificate Status Protocol):</strong>\nReal-time revocation check. Browser asks the CA's OCSP server "is cert serial #XYZ still valid?" CA responds: good, revoked, or unknown.\nPrivacy problem: the CA sees every site you visit (it sees every OCSP request).\n\n<strong>OCSP Stapling:</strong>\nServer fetches its own OCSP response from the CA, caches it, and staples (attaches) it to the TLS handshake. Browser gets revocation proof from the server directly, without contacting the CA.\nBenefits: faster (no extra round trip), private (CA doesn't see client IPs), works even if CA's OCSP server is down.\n\n<strong>CRL vs OCSP:</strong>\nCRL (Certificate Revocation List): a big list the CA publishes. Download it, check if your cert is on it. Slow, large, often cached and stale.\nOCSP: real-time query for one specific cert. Faster, but privacy issue (fixed by stapling).`,
        memory: `CT log = public registry of every ID card ever issued. If a fake ID gets made, someone will notice it in the registry.\n\nOCSP stapling = instead of you calling the DMV to verify a license (slow, private info leak), the driver pre-fetches a notarized "this license is valid" stamp and hands it to you directly.`,
        examTip: `CT = every cert logged publicly, catches misissuance. SCT = proof the cert is in a log (required by Chrome). OCSP stapling = server fetches revocation status, sends it in handshake = faster + private. CRL = big list, stale. OCSP without stapling = CA sees your browsing. Pinning = risky in public sites, used in mobile apps.`,
        facts: ["CT=public cert log", "SCT=log proof in cert", "OCSP=real-time revocation", "Stapling=server fetches OCSP", "CRL=big revocation list", "Pinning=specific cert enforced", "DigiNotar=CT motivation"]
      },

      {
        ...
        tags: ["sec", "arch"],
        chain: ["Weak input (password or secret)", "KDF stretches and strengthens it", "Salt added for uniqueness", "Output = strong cryptographic key", "Brute-force made impractical"],
        blurb: "Passwords are weak. KDFs turn them into proper cryptographic keys by adding computational cost and randomness so that cracking one hash takes seconds to years.",
        detail: `<strong>The problem:</strong>\nUser passwords are low-entropy strings ("Summer2024!"). You can't use them directly as cryptographic keys. A KDF transforms a password + salt into a key suitable for encryption or storage.\n\n<strong>Password hashing KDFs (designed to be SLOW):</strong>\n\n• <strong>bcrypt:</strong> adaptive cost factor. Default 10 rounds ≈ ~100ms. Raise the cost as CPUs get faster. Max password length = 72 bytes (limitation). Widely used, well-understood. Output includes the salt + cost factor.\n\n• <strong>scrypt:</strong> memory-hard. Designed to be expensive in both CPU AND memory — defeats GPU and ASIC cracking (they're limited by memory bandwidth, not compute). Parameters: N (CPU cost), r (memory), p (parallelism).\n\n• <strong>Argon2:</strong> winner of the Password Hashing Competition (2015). Three variants:\n  - Argon2d: data-dependent memory access (fast, but side-channel risk)\n  - Argon2i: data-independent (side-channel resistant)\n  - Argon2id: hybrid — recommended default. Configurable time, memory, and parallelism. Modern first choice.\n\n• <strong>PBKDF2:</strong> NIST-approved, FIPS-compliant. Just applies HMAC many times (iterations). Less memory-hard than the others — more susceptible to GPU attacks. Still required in government/regulated contexts.\n\n<strong>Key derivation KDFs (for deriving keys from other secrets, not passwords):</strong>\n\n• <strong>HKDF (HMAC-based Extract-and-Expand KDF):</strong> takes a shared secret (e.g., from ECDHE) and derives multiple keys. Two phases: Extract (combine secret + salt → pseudorandom key), then Expand (stretch that into as many keys as needed). Used inside TLS 1.3 to derive handshake keys and application keys from the ECDHE output.\n\n<strong>Salt:</strong>\nA random value added before hashing — unique per user. Even if two users have the same password, their hashes differ. Defeats rainbow tables and pre-computation attacks. Must be stored alongside the hash (it's not secret, just unique).`,
        memory: `KDF = the difference between a deadbolt and a screen door. Passwords alone = screen door. Password through Argon2 = deadbolt plus a moat.\n\nbcrypt = the old reliable. Argon2id = the modern upgrade. PBKDF2 = the government's paperwork-approved option. HKDF = for expanding secrets inside protocols, not for passwords.`,
        examTip: `Argon2id = recommended for new systems. bcrypt = widely deployed, still fine. PBKDF2 = FIPS-compliant, weaker against GPUs. scrypt = memory-hard. HKDF = key derivation inside protocols (TLS 1.3 uses it). Salt = unique per user, stored in plaintext with hash. Pepper = secret value added to all hashes (stored separately from DB).`,
        facts: ["Argon2id=recommended", "bcrypt=adaptive cost", "scrypt=memory-hard", "PBKDF2=FIPS-compliant", "HKDF=protocol key derivation", "Salt=unique per user", "Slow=intentional design"]
      },

      {
        ...
        tags: ["sec", "arch"],
        chain: ["Key generated", "Distributed securely", "Stored protected", "Used for encryption/signing", "Rotated on schedule", "Retired and destroyed"],
        blurb: "Cryptography is only as strong as its key management. Most real-world crypto failures aren't algorithm failures — they're key management failures.",
        detail: `<strong>The key lifecycle — every phase matters:</strong>\n\n• <strong>Generation:</strong> keys must come from a cryptographically secure random number generator (CSPRNG). Weak entropy = predictable keys. Use OS-provided CSPRNG (/dev/urandom on Linux, CryptGenRandom on Windows). Never seed a PRNG with timestamps or predictable values.\n\n• <strong>Distribution:</strong> how does the key get from generator to user? Options: in-person, asymmetric wrapping (encrypt the key with recipient's public key), key agreement protocol (DH). Never transmit keys in plaintext.\n\n• <strong>Storage:</strong> keys at rest must be protected. Options: HSM, TPM, encrypted key store, OS keychain. Never store keys alongside the data they protect. Never hard-code keys in source code (common mistake, caught by tools like truffleHog).\n\n• <strong>Usage:</strong> enforce purpose limits — a key used for signing shouldn't be used for encryption. Track usage. Implement access controls around who/what can use each key.\n\n• <strong>Rotation:</strong> regularly replace keys even without a known compromise. Limits exposure window if a key is silently compromised. Frequency depends on risk — TLS session keys rotate every session; certificate keys might rotate yearly. Automated rotation reduces human error.\n\n• <strong>Revocation:</strong> immediate invalidation when a key is suspected compromised. For certificates: CRL / OCSP. For symmetric keys: revoke in the key management system. Any data encrypted with the revoked key may need re-encryption.\n\n• <strong>Archival:</strong> encrypted backups of keys needed to decrypt old data. Must be as protected as live keys.\n\n• <strong>Destruction:</strong> cryptographic erasure — overwrite key material multiple times so it can't be recovered. For hardware: degauss, shred, or destroy the storage device.\n\n<strong>Key wrapping:</strong>\nEncrypting one key with another key (a Key Encryption Key / KEK). The KEK protects the data encryption keys (DEKs). Rotate DEKs frequently; protect the KEK very carefully. This is how AWS KMS and Azure Key Vault work — your data key is wrapped by a master key that never leaves the service.\n\n<strong>Key escrow:</strong>\nA copy of the key held by a trusted third party for recovery. Required in some regulated industries. Controversial because it creates a high-value target.`,
        memory: `Key management = like managing master keys to a building. Generate them carefully (locksmith, not a random person). Never copy them unnecessarily. Store in a safe, not a desk drawer. Change the locks periodically. When someone leaves, change the locks immediately. Destroy old keys so nobody finds them.\n\nMost crypto breaches aren't "AES was broken." They're "the key was in the source code on GitHub."`,
        examTip: `Key rotation = limits breach window. Key wrapping = encrypt keys with KEKs (how cloud KMS works). Destruction = cryptographic erasure. Hard-coded keys in source code = most common real-world mistake. Key escrow = held by third party for recovery, creates risk. Separation: never store key with the data it encrypts.`,
        facts: ["CSPRNG=required for key gen", "Never hard-code keys", "Rotate keys periodically", "KEK wraps DEKs", "Cryptographic erasure=destruction", "Escrow=third-party copy", "Distribution=encrypt with recipient pubkey"]
      },

      {
        id: "hsm-tpm",
        title: "HSM & TPM — Hardware Roots of Trust",
        tags: ["sec", "arch"],
        chain: ["Key generated inside hardware", "Never exported in plaintext", "Operations performed inside the device", "Tamper-evident/resistant casing", "Physical attack = key destroyed"],
        blurb: "Hardware security modules and TPMs move key storage and crypto operations into tamper-resistant hardware. The private key literally cannot leave the device — even if the OS is compromised.",
        detail: `<strong>HSM (Hardware Security Module):</strong>\nA dedicated, hardened hardware device that generates, stores, and uses cryptographic keys. Keys never leave the HSM in plaintext — all operations (sign, decrypt, wrap) happen inside the device.\n\nKey properties:\n• Tamper-evident: physical evidence if someone tries to open it\n• Tamper-resistant: mechanisms that destroy key material if tampered with (epoxy, sensors)\n• FIPS 140-2/3 validated: certified by NIST for cryptographic strength\n• Performance: hardware-accelerated crypto — can do thousands of RSA operations per second\n\nUse cases: CA root key protection, code signing keys, payment processing (PCI DSS often requires HSM), TLS private keys on high-traffic servers, database encryption key management.\n\nCloud HSMs: AWS CloudHSM, Azure Dedicated HSM — you get your own HSM in the cloud. Different from regular KMS (where the provider manages keys on your behalf).\n\n<strong>TPM (Trusted Platform Module):</strong>\nA chip embedded on the motherboard of most modern computers and servers. Cheaper and less capable than a full HSM, but provides a hardware root of trust on the endpoint.\n\nKey capabilities:\n• Stores keys and secrets in hardware\n• Sealing: encrypt data such that it can only be decrypted on THIS specific machine in THIS specific state\n• Platform attestation: prove to a remote server that this machine's firmware and OS haven't been tampered with (used in Zero Trust device health checks)\n• Measured boot: TPM records hash of each boot component; if anything changes, it's detectable\n\n<strong>Secure Enclave / TEE (Trusted Execution Environment):</strong>\nSoftware-defined isolated execution environment inside a main CPU. Intel SGX, ARM TrustZone, Apple Secure Enclave. Code and data inside are isolated from the main OS — even the kernel can't read it. Used for Face ID processing, Apple Pay, sensitive key operations on mobile.\n\n<strong>Why it matters architecturally:</strong>\nIf your private key is a file on disk (even encrypted), a sufficiently privileged attacker who compromises the OS can eventually extract it. With an HSM or TPM, there's no file to steal — the key is trapped in hardware.`,
        memory: `HSM = a bank vault for keys. You tell the vault "sign this," it signs it and hands back the signature. The key never comes out of the vault.\n\nTPM = a small safe built into your laptop's motherboard. It knows if someone replaced the hard drive or boot loader since last time. Used by BitLocker to refuse to unlock if the machine was tampered with.`,
        examTip: `HSM = keys never leave in plaintext, tamper-resistant, FIPS 140-2/3. Required for CA root keys, PCI HSM requirement. TPM = embedded chip, measured boot, device attestation, sealing. BitLocker uses TPM. Cloud KMS ≠ CloudHSM — KMS is shared infrastructure, CloudHSM is dedicated. TEE = software isolation inside CPU.`,
        facts: ["HSM=keys never exported", "FIPS 140-2/3=HSM cert", "TPM=motherboard chip", "Sealing=tied to machine state", "Measured boot=TPM records hashes", "BitLocker=uses TPM", "TEE=CPU-level isolation"]
      },

      {
        id: "crypto-attacks",
        title: "Attacks on Cryptography",
        tags: ["sec", "arch"],
        chain: ["Crypto deployed incorrectly", "Or algorithm has weakness", "Attacker probes the gap", "Extracts plaintext or key", "Without ever breaking the math"],
        blurb: "Most crypto breaks at the seams — wrong mode, reused nonce, timing leak, weak protocol. Understanding attack classes tells you exactly where not to cut corners.",
        detail: `<strong>Birthday Attack:</strong>\nExploits the birthday paradox: you need far fewer tries than expected to find two inputs with the same hash (collision). For an n-bit hash, you only need ~2^(n/2) attempts.\nMD5 (128-bit): collisions found after ~2^64 operations — trivially fast with modern GPUs.\nSHA-1 (160-bit): collision demonstrated by Google (SHAttered, 2017). Broken.\nSHA-256 (256-bit): ~2^128 for collision — computationally infeasible. Safe.\nPractical impact: forged digital signatures, duplicate TLS certificates.\n\n<strong>Padding Oracle Attack:</strong>\nExploits the error messages returned when block cipher padding is invalid. Attacker modifies ciphertext bytes, sends to server, reads whether it decrypts with valid or invalid padding. By doing this systematically, they can decrypt the entire ciphertext without the key.\nAffected: CBC mode in SSL 3.0 (POODLE), TLS 1.0 (BEAST), many web frameworks.\nFix: AEAD (AES-GCM) — authentication check fails before padding is ever examined.\n\n<strong>Timing Attack (Side-Channel):</strong>\nCryptographic operations take measurably different time depending on the key or secret. By measuring response times precisely, an attacker can recover key bits.\nClassic example: naive string comparison stops at the first mismatched byte — "aaa..." fails faster than "correct_horse_battery_staple". This leaks how many characters were correct.\nFix: constant-time comparison functions. All modern crypto libraries use these for secret comparisons.\nOther side-channels: power consumption (power analysis), electromagnetic emissions, cache timing (Spectre/Meltdown).\n\n<strong>Downgrade Attack:</strong>\nAttacker intercepts the TLS handshake and manipulates it to force both parties to negotiate a weaker protocol or cipher suite that the attacker can then break.\nExamples: POODLE forced SSL 3.0. FREAK forced 512-bit RSA export keys. LOGJAM forced 512-bit DH.\nFix: TLS 1.3 removes all weak options — nothing to downgrade to. Also: HSTS preloading, minimum version enforcement.\n\n<strong>Chosen-Plaintext Attack (CPA):</strong>\nAttacker can choose arbitrary plaintexts and obtain their encryptions. Goal: recover the key or decrypt other ciphertexts.\nAll modern ciphers are designed to be CPA-secure (the attacker learning ciphertext for chosen plaintexts reveals nothing useful).\nWhy it matters: if your system lets attackers encrypt arbitrary data (e.g., via an oracle), CPA-security is the minimum bar.\n\n<strong>Chosen-Ciphertext Attack (CCA):</strong>\nAttacker can also submit ciphertexts and get them decrypted (except the target). Much stronger attack model.\nPadding oracle = a CCA. A system that decrypts attacker-supplied ciphertexts and reveals error info = CCA vulnerable.\nFix: authenticated encryption (AEAD) — attacker can't get modified ciphertexts decrypted because the auth tag will fail.\n\n<strong>Length Extension Attack:</strong>\nAffects hash constructions like MD5(secret || message). Attacker can append data and compute a valid hash without knowing the secret.\nAffects: MD5, SHA-1, SHA-256 (all Merkle-Damgård constructions) when used naively as MACs.\nFix: HMAC (the nested key construction specifically prevents this). SHA-3 / BLAKE3 are inherently immune.\n\n<strong>Replay Attack:</strong>\nAttacker captures a valid authenticated message and retransmits it later. The message is cryptographically valid — but stale.\nFix: include a nonce (number used once), timestamp, or sequence number in the authenticated data. TLS handles this with record sequence numbers.`,
        memory: `Birthday = hash collisions come faster than you think (sqrt of keyspace, not full keyspace).\nPadding oracle = error messages are a decryption oracle. AEAD = no oracle.\nTiming = "how long did that take?" leaks secrets. Use constant-time code.\nDowngrade = force old weak protocol. TLS 1.3 = nothing weak to fall back to.\nLength extension = MD5/SHA-256 MAC done wrong. Use HMAC.`,
        examTip: `Birthday attack → use 256-bit+ hashes. Padding oracle → use AEAD not CBC. Timing attack → constant-time comparisons. Downgrade → enforce TLS 1.2+ minimum, use TLS 1.3. Length extension → use HMAC not bare hash. Replay → nonces/timestamps. Most real crypto failures = implementation errors, not algorithm breaks.`,
        facts: ["Birthday=2^(n/2) collisions", "MD5+SHA-1=broken", "Padding oracle=CBC risk", "AEAD=eliminates oracle", "Timing=side-channel", "Downgrade=force weak cipher", "Length extension=HMAC fixes", "Replay=nonce/timestamp fix"]
      },

      {
        ...
        tags: ["arch"],
        chain: ["Quantum computers scale up", "Shor's algorithm breaks RSA/ECC", "Grover's halves symmetric key strength", "NIST selects PQC standards", "Crypto agility = swap without rebuilding"],
        blurb: "A sufficiently large quantum computer would break every asymmetric algorithm in use today. The transition to post-quantum cryptography is already underway — architects need to plan for it now.",
        detail: `<strong>Why quantum threatens current crypto:</strong>\n\n• <strong>Shor's Algorithm:</strong> runs on a quantum computer. Solves the integer factorization problem (RSA) and discrete logarithm problem (DH, ECC) in polynomial time. A quantum computer with enough stable qubits could break RSA-2048 or ECC-256 in hours.\nImpact: ALL current asymmetric cryptography (RSA, ECC, DH, DSA, ECDSA) is broken.\n\n• <strong>Grover's Algorithm:</strong> speeds up brute-force search of symmetric keys. Reduces effective key strength by half (AES-256 → equivalent to AES-128 against a quantum attacker).\nImpact: symmetric crypto weakened but not broken. Fix: double key sizes. AES-256 remains safe post-quantum.\n\n<strong>Harvest now, decrypt later (HNDL):</strong>\nAdversaries are already recording encrypted traffic today, planning to decrypt it once quantum computers are available. For long-lived secrets (state secrets, medical data, 20-year infrastructure), the threat is NOW — even though quantum computers aren't ready yet.\n\n<strong>NIST PQC Standards (finalized 2024):</strong>\n\n• <strong>ML-KEM (CRYSTALS-Kyber):</strong> key encapsulation mechanism — replaces RSA/ECDH for key exchange. Based on module learning with errors (MLWE) lattice problem. Fast, small keys and ciphertexts. Primary standard.\n\n• <strong>ML-DSA (CRYSTALS-Dilithium):</strong> digital signatures. Also lattice-based (MLWE/MSIS). Primary signature standard.\n\n• <strong>SLH-DSA (SPHINCS+):</strong> signature algorithm based on hash functions (not lattice math). Larger signatures but different mathematical foundation — provides diversity if lattice problems are broken.\n\n• <strong>FN-DSA (FALCON):</strong> compact lattice-based signatures. Smaller than Dilithium but trickier to implement correctly.\n\n<strong>Hybrid cryptography (transition strategy):</strong>\nRun classical (ECDHE) AND post-quantum (Kyber) key exchange simultaneously. Combine the shared secrets. Only broken if an attacker breaks BOTH. Used in TLS 1.3 hybrid mode — Chrome and Cloudflare already doing this.\n\n<strong>Crypto agility:</strong>\nDesign systems so you can swap cryptographic algorithms without a full rebuild. Abstract the crypto layer. Use algorithm identifiers in protocols. Don't hard-code algorithm choices deep in the stack. The TLS cipher suite negotiation is an example of crypto agility in practice.`,
        memory: `Quantum = kryptonite for RSA and ECC. Shor's algorithm = finds the secret factors / discrete log that classical computers can't. AES-256 survives (just use it, not AES-128).\n\nHarvest now decrypt later = they're already recording your traffic. Long-lived secrets need PQC protection TODAY, not when quantum computers arrive.\n\nKyber = new key exchange. Dilithium = new signatures. Both replace what RSA/ECC do, just using math quantum computers can't speed up.`,
        examTip: `Shor's = breaks RSA/ECC/DH. Grover's = weakens symmetric (double key sizes). AES-256 = post-quantum safe. NIST PQC: Kyber (key exchange), Dilithium (signatures), SPHINCS+ (hash-based sigs). HNDL = harvest now decrypt later = current threat. Crypto agility = swap algorithms without redesigning the system.`,
        facts: ["Shor's=breaks RSA/ECC", "Grover's=weakens symmetric", "AES-256=PQ safe", "Kyber=PQ key exchange", "Dilithium=PQ signatures", "HNDL=current threat", "Crypto agility=swap ready", "Hybrid=classical+PQ together"]
      },

      {
        ...
        tags: ["arch"],
        chain: ["Prover knows a secret", "Wants to convince Verifier", "Without revealing the secret itself", "Mathematical protocol executed", "Verifier convinced — learns nothing else"],
        blurb: "Prove you know something without revealing it. ZKPs sound impossible but they're real — and they're the foundation of privacy-preserving identity, blockchain scaling, and anonymous authentication.",
        detail: `<strong>The core idea:</strong>\nAlice wants to prove to Bob that she knows a secret (a password, a private key, membership in a group) without revealing the secret itself. A zero-knowledge proof lets her do exactly that.\n\nA ZKP must satisfy three properties:\n• <strong>Completeness:</strong> if the statement is true, an honest prover can always convince the verifier.\n• <strong>Soundness:</strong> if the statement is false, no cheating prover can convince the verifier (except with negligible probability).\n• <strong>Zero-knowledge:</strong> the verifier learns nothing beyond the fact that the statement is true.\n\n<strong>Classic analogy — the cave:</strong>\nA circular cave has a magic door in the middle requiring a secret word. Alice wants to prove she knows the word without saying it. She enters from one side while Bob waits outside. Bob shouts which side he wants her to exit from. She always exits the correct side. After many rounds, Bob is convinced she knows the word — but never heard it.\n\n<strong>Interactive vs Non-Interactive ZKPs:</strong>\nInteractive: prover and verifier exchange multiple rounds of messages (like the cave). Requires both parties online simultaneously.\nNon-interactive (NIZK): prover produces a single proof that anyone can verify later, without interaction. Enabled by the Fiat-Shamir transform or a Common Reference String (CRS).\n\n<strong>zk-SNARKs (Succinct Non-interactive ARguments of Knowledge):</strong>\nThe most prominent ZKP system today. Properties:\n• Succinct: proof is tiny (a few hundred bytes) regardless of computation size\n• Non-interactive: one proof, verified by anyone\n• Fast to verify (milliseconds)\n• Requires a trusted setup (a "ceremony" to generate public parameters — if setup is compromised, false proofs possible)\nUsed in: Zcash (private cryptocurrency), Ethereum L2 rollups (zkSync, StarkNet), anonymous credential systems.\n\n<strong>zk-STARKs:</strong>\nLike SNARKs but: no trusted setup required, post-quantum secure, larger proofs. Used in StarkNet.\n\n<strong>Practical security applications:</strong>\n• Age verification without revealing birthdate\n• Proof of sufficient funds without revealing account balance\n• Password authentication without transmitting the password\n• Membership in a group without revealing identity\n• Blockchain transaction validity without revealing transaction details`,
        memory: `ZKP = proving you know the combination to a safe by opening it while blindfolded, without ever touching the dial where someone can see the numbers. You opened it → you know it. Nobody saw how.\n\nzk-SNARK = a tiny receipt that proves you did a massive computation correctly. The receipt is the same size regardless of how big the computation was.`,
        examTip: `ZKP properties: completeness, soundness, zero-knowledge. zk-SNARKs = succinct, non-interactive, need trusted setup. zk-STARKs = no trusted setup, post-quantum, larger proofs. Real-world: blockchain privacy (Zcash), L2 scaling (zkSync), anonymous credentials. The core idea: prove knowledge without revealing the knowledge itself.`,
        facts: ["ZKP=prove without revealing", "Completeness+Soundness+ZK=3 props", "zk-SNARK=tiny proof", "Trusted setup=SNARK requirement", "zk-STARK=no trusted setup", "PQ-safe=STARKs", "Zcash=ZKP in production"]
      },

      {
        ...
        tags: ["arch"],
        chain: ["Data encrypted", "Sent to untrusted server", "Server computes on ciphertext", "Returns encrypted result", "Client decrypts — correct answer, server saw nothing"],
        blurb: "Compute on encrypted data without ever decrypting it. The server processes your ciphertext and returns an encrypted result — it learns nothing about your data or the answer.",
        detail: `<strong>The problem it solves:</strong>\nYou want to use a cloud service to process sensitive data (medical records, financial data, private queries). But you don't trust the cloud provider. Normally: encrypt before upload = can't process. Decrypt on their servers = they see everything. Homomorphic encryption = third option: they process the encrypted data without ever seeing it.\n\n<strong>How it works (conceptually):</strong>\nA homomorphic encryption scheme has a special property: operations on ciphertexts correspond to operations on plaintexts.\nEnc(a) + Enc(b) = Enc(a + b)\nEnc(a) × Enc(b) = Enc(a × b)\nThe server computes on the ciphertext. Client decrypts the result. Correct answer. Server saw only noise.\n\n<strong>Types:</strong>\n• <strong>Partially Homomorphic (PHE):</strong> supports only one type of operation (addition OR multiplication), unlimited times. Example: RSA is multiplicatively homomorphic. Paillier cryptosystem is additively homomorphic. Used in some voting systems and private set intersection.\n\n• <strong>Somewhat Homomorphic (SHE):</strong> supports both addition and multiplication but only a limited number of operations before noise accumulates and corrupts the result.\n\n• <strong>Fully Homomorphic (FHE):</strong> supports arbitrary computation — any function can be evaluated on ciphertexts, unlimited times. Theoretical breakthrough by Craig Gentry in 2009. Uses "bootstrapping" to reset accumulated noise.\n\n<strong>Current state:</strong>\nFHE is real and implemented (Microsoft SEAL, OpenFHE, TFHE) but computationally expensive — operations on ciphertext take 1000–10,000× longer than on plaintext. Active research area; getting faster every year.\n\n<strong>Real applications today:</strong>\n• Private information retrieval: query a database without the server knowing what you queried\n• Genomic analysis on encrypted patient DNA\n• Machine learning inference on encrypted data (banks running fraud detection on encrypted transactions)\n• Secure multi-party computation in financial systems\n\n<strong>Relationship to ZKPs and MPC:</strong>\nAll three are "privacy-preserving computation" techniques. FHE = one party computes on another's encrypted data. ZKP = prove a computation was done correctly. MPC = multiple parties jointly compute without revealing inputs to each other.`,
        memory: `FHE = a locked calculator. You hand someone a locked box (encrypted data), they use a special calculator to get a result (also locked), hand it back. You unlock the result. They never saw your numbers or the answer.\n\nIt sounds like magic. It is math. And it's slow — like "takes a minute to add two numbers" slow. But it's getting faster fast.`,
        examTip: `FHE = compute on encrypted data, server sees nothing. PHE = only one operation (add or multiply). FHE = any computation. Currently slow but usable for specific workloads. Gentry 2009 = first FHE scheme. Real libraries: Microsoft SEAL, OpenFHE. Distinct from ZKP (prove correctness) and MPC (joint computation between parties).`,
        facts: ["FHE=compute on ciphertext", "PHE=one operation type", "Gentry 2009=first FHE", "1000x slower than plaintext", "SEAL/OpenFHE=real libraries", "No decryption needed server-side", "ZKP+MPC+FHE=privacy-preserving trio"]
      },

      {
        ...
        tags: ["arch"],
        chain: ["Secret too valuable for one person", "Split into N shares", "Any M shares reconstruct it", "No M-1 shares reveal anything", "Distributed trust achieved"],
        blurb: "Split a secret so no single person holds it. Require any 3-of-5 shares to reconstruct it. Lose 2 shares entirely — secret is still safe. This is how HSM backup keys, nuclear launch codes, and crypto wallet recovery work.",
        detail: `<strong>The problem:</strong>\nA master key or critical secret held by one person is a single point of failure AND a single point of compromise. What if they're unavailable? What if they're malicious? Secret sharing distributes both the risk and the trust.\n\n<strong>Shamir's Secret Sharing (SSS):</strong>\nDeveloped by Adi Shamir (the S in RSA) in 1979. Based on polynomial interpolation.\n\nHow it works:\n1. Choose threshold M (minimum shares to reconstruct) and total shares N\n2. Embed the secret as the constant term of a random polynomial of degree M-1\n3. Evaluate the polynomial at N different points — each point is one share\n4. Any M points uniquely determine the polynomial (and recover the secret)\n5. Any M-1 points reveal ZERO information about the secret (information-theoretic security)\n\nExample: 3-of-5 scheme. Give shares to 5 executives. Any 3 can reconstruct the root CA key. Even if 2 are compromised, the attacker learns nothing. Even if 2 are lost, the secret is still recoverable.\n\n<strong>Perfect security property:</strong>\nUnlike password-based schemes, SSS has information-theoretic security — holding M-1 shares gives you literally zero mathematical information about the secret. Not "hard to compute" — mathematically impossible to learn anything.\n\n<strong>Real-world uses:</strong>\n• Root CA key ceremonies (CA root keys split among several trusted persons, stored on HSMs)\n• Cryptocurrency wallet recovery\n• Nuclear launch authorization systems\n• Master password storage for critical infrastructure\n• DNSSEC root key — the DNS root KSK is protected with a 7-of-14 scheme across trusted community representatives worldwide\n\n<strong>Threshold signatures (vs secret sharing):</strong>\nInstead of reconstructing the key and then signing, threshold signature schemes (like FROST for Ed25519) allow M-of-N parties to jointly produce a valid signature without any single party ever holding the full key. Key never assembled in one place — stronger security.\n\n<strong>Multi-Party Computation (MPC):</strong>\nGeneralization: N parties jointly compute a function on their private inputs without revealing those inputs to each other. Banks use MPC for fraud detection across institutions without sharing raw transaction data.`,
        memory: `Shamir's = tearing a treasure map into 5 pieces. You need any 3 pieces to find the treasure. With only 2 pieces, you have no idea where to even start looking — not just "hard to find," mathematically impossible to know.\n\nDNSSEC root key = the most important key on the internet, protected by 7-of-14 trusted humans across 7 countries. Lose 7 of them → still safe. Need to sign → fly them all to a secure facility.`,
        examTip: `Shamir's Secret Sharing: M-of-N threshold, polynomial interpolation, information-theoretic security (M-1 shares = zero info). DNSSEC root = real-world 7-of-14 SSS. Threshold signatures = sign without assembling key. MPC = joint computation, private inputs. SSS is perfect secrecy — not computational, mathematical.`,
        facts: ["Shamir's=polynomial interpolation", "M-of-N threshold", "M-1 shares=zero information", "Information-theoretic security", "DNSSEC root=7-of-14", "Threshold sigs=key never assembled", "MPC=joint private computation"]
      },
